<script>
  ...
  const addTimeRecord = (type) => {
    ...
    listTimeRecords = [...listTimeRecords, { date: timeOutput, type: type }];
  };
  ...
  const onClickList = () => {
    for (let i = 0; i < listTimeRecords.length; i++) {
      if (listTimeRecords[i].type === "P20") {
        for (let j = 0; j < listTimeRecords.length; j++) {
            console.log(listTimeRecords[j]);
          $timeRecordsStore = [...$timeRecordsStore, listTimeRecords[j]];
        }
      }
    }
  };
